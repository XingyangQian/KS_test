...
def RT_correlation(rt1=None, rt2=None, rt_corr=None, coverage_cutoff=0, background_base_density=0):
	...
	subprocess.call(["%s -1 %s -2 %s -T %s -b %s > %s"%(correlationRT_pl, rt1, rt2, coverage_cutoff, background_base_density, rt_corr)],shell=True)
	gj.printFuncRun('RT_correlation')
...
def RT_combine(rt1=None, rt2=None, rt_comb=None):
	...
	subprocess.call(["%s -i %s:%s -o %s"%(combineRT_pl, rt1, rt2, rt_comb)],shell=True)
	gj.printFuncRun('RT_combine')
...
def RT_normalize(rt=None):
	...
	subprocess.call(["%s -i %s -o %s -m mean:vigintile2 -d 32 -l 32"%(normalizeRT_pl, rt, normalize_rt)],shell=True)
	gj.printFuncRun('RT_normalize')
...
def calc_enrich(f_normalized_rt=None, b_normalized_rt=None, icshape_tmp_out=None, x=0.25):
	...
	subprocess.call(["%s -f %s -b %s -o %s -w factor5:scaling1 -x %s"%(calc_enrich_pl, f_normalized_rt, b_normalized_rt, icshape_tmp_out, x)],shell=True)
	gj.printFuncRun('calc_enrich')
